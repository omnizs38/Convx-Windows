...
/** Runtime effects are expensive to compile, so each shader is built once per process. */
internal object GlassShaders {
    val refraction: RuntimeEffect by lazy { RuntimeEffect.makeForShader(RoundedRectRefractionShaderString) }
    val refractionWithDispersion: RuntimeEffect by lazy {
        RuntimeEffect.makeForShader(RoundedRectRefractionWithDispersionShaderString)
    }
    val highlight: RuntimeEffect by lazy { RuntimeEffect.makeForShader(DefaultHighlightShaderString) }
}
